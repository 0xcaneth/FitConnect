...
struct SimpleUserProfileView: View {
    ...
    private func profileContent(user: FitConnectUser) -> some View {
        VStack(spacing: 24) {
            ...
            // Stats
            HStack(spacing: 0) {
                SimpleStatView(title: "XP", value: "\(user.xp ?? 0)")
                
                Divider()
                    .background(Color.white.opacity(0.2))
                    .frame(height: 40)
                
                SimpleStatView(title: "Level", value: "\(user.level ?? 1)")
                
                Divider()
                    .background(Color.white.opacity(0.2))
                    .frame(height: 40)
                
                SimpleStatView(title: "Plan", value: user.subscription?.rawValue.capitalized ?? "Free")
            }
            .padding(.horizontal)
            .padding(.vertical, 16)
            .background(
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 12) {
                    SimpleInfoRow(title: "Email", value: user.email)
                    
                    if let fitnessGoal = user.fitnessGoal, !fitnessGoal.isEmpty {
                        SimpleInfoRow(title: "Fitness Goal", value: fitnessGoal)
                    }
                    
                    if let activityLevel = user.activityLevel, !activityLevel.isEmpty {
                        SimpleInfoRow(title: "Activity Level", value: activityLevel)
                    }
                    
                    if let age = user.age {
                        SimpleInfoRow(title: "Age", value: "\(age) years old")
                    }
                    
                    if let weight = user.weight {
                        SimpleInfoRow(title: "Weight", value: String(format: "%.1f kg", weight))
                    }
                    
                    if let height = user.height {
                        SimpleInfoRow(title: "Height", value: String(format: "%.0f cm", height))
                    }
                }
            }
