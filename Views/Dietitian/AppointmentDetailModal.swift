...
@available(iOS 16.0, *)
struct AppointmentDetailModal: View {
    ...
    var body: some View {
        ...
        .alert("Cancel Appointment", isPresented: $showCancelConfirmation) {
            ...
            Text("Are you sure you want to cancel this appointment with \(appointment.clientName ?? "this client")? This action cannot be undone.")
        }
    }
    ...
    private var headerView: some View {
        ...
        .background(
            LinearGradient(
                colors: [FitConnectColors.accentPurple.opacity(0.3), Color.clear],
                startPoint: .top,
                endPoint: .bottom
            )
        )
    ...
    private var clientInfoSection: some View {
        HStack(spacing: 16) {
            ...
            ZStack {
                ...
                Text((appointment.clientName?.first?.uppercased()) ?? "C")
                    .font(.system(size: 24, weight: .bold))
                    .foregroundColor(.white)
            }
        ...
        VStack(spacing: 0) {
            ...
            VStack(spacing: 0) {
                ...
                if appointment.isUpcoming() {
                    ...
                    // Make the Date row the last one if appointment is not upcoming
                    Rectangle()
                        .fill(Color.clear)
                        .frame(height: 0)
                }
...
#if DEBUG
@available(iOS 16.0, *)
struct AppointmentDetailModal_Previews: PreviewProvider {
